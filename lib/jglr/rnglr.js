define(["./cyclicJSON"], function(cycle) {
  ...
  Atom.fromSerializable = function(obj) {
    ...
    if (obj === "%EPS") return EPSILON;
    if (typeof obj === "string") return new Nonterm(obj);
    if (obj instanceof Array) return new Token(obj[0], obj[1] || obj[0]);
    return null;
  }
  ...
  Nonterm.prototype.toSerializable = function() { return this.name; }
  ...
  const EPSILON = Object.create(Atom.prototype, 
                                {name: {enumerable: true, value: "EPSILON"}, 
                                 ...
                                 toSerializable: {value: function() { return "%EPS"; } }});
  ...
  Rule.fromSerializable = function(rulesByOldId, id) {
    ...
    if (obj[0] instanceof Array) {
      var lookahead = obj[0];
      var pos = obj[1];
      var base = Rule.fromSerializable(rulesByOldId, obj[2]);
      return RuleFactory.make(base.name, base.symbols, Atom.fromSerializable(lookahead), pos);
    } else {
      var name = obj[0];
      var pos = obj[1];
      var syms = obj[2].map(function(s) { return Atom.fromSerializable(s); });
      return RuleFactory.make(name, syms, undefined, pos);
    }
  }
  ...
  Rule.prototype = {
    ...
    toSerializable: function() {
      if (this.lookahead) {
        return [this.lookahead.toSerializable(), this.position, this.withLookaheadPosition(undefined, 0).id];
      } else {
        return [this.name, this.position, this.symbols.map(function(s) { return s.toSerializable(); })];
      }
    }
  }
  ...
  Grammar.fromSerializable = function(obj) {
    ...
    for (var i = 0; i < obj.eSPPFs.length; i++) {
      ...
      var srcRule = typeof src[1] === "number" ? src[1] : src[2];
      var srcKids = src[1] instanceof Array ? src[1] : undefined;
      if (srcKids) {
        var kids = [];
        for (var j = 0; j < srcKids.length; j++)
          kids.push(g.eSPPFs[srcKids[j]].null);
        dest.addChildren(rulesByOldId[srcRule], kids, g);
      }
    }
    return g;
  }
    ...
    toSerializable: function() {
      ...
      for (var i = 0; i < this.eSPPFs.length; i++) {
        ...
        if (null_eSPPF.kids) {
          ...
          if (null_eSPPF.rule)
            ret.eSPPFs[i] = [label, kids, null_eSPPF.rule.id];
          else
            ret.eSPPFs[i] = [label, kids];
        } else {
          if (null_eSPPF.rule)
            ret.eSPPFs[i] = [label, null_eSPPF.rule.id];
          else
            ret.eSPPFs[i] = [label];
        }
      }
            ...
            for (var j = 0; j < name_rule.symbols.length; j++) {
              if (name_rule.symbols[j] instanceof Nonterm) {
                ...
                if (this.nontermFirst[name_rule.symbols[j]] === undefined)
                  throw("Couldn't find a nontermFirst for " + name_rule.symbols[j]);
